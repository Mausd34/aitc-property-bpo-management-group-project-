...
class Property(models.Model):
 client=models.ForeignKey(Client,on_delete=models.PROTECT,related_name='properties'); address=models.TextField(); property_type=models.CharField(max_length=80,blank=True); details=models.TextField(blank=True); status=models.CharField(max_length=50,default='Active'); image=models.ImageField(upload_to='properties/',blank=True,null=True)
...
class WorkOrder(models.Model):
 ...
 client=models.ForeignKey(Client,on_delete=models.PROTECT,related_name='work_orders'); property=models.ForeignKey(Property,on_delete=models.PROTECT,related_name='work_orders'); title=models.CharField(max_length=200); work_type=models.CharField(max_length=100,blank=True); assigned_to=models.CharField(max_length=150,blank=True); description=models.TextField(blank=True); priority=models.CharField(max_length=20,default='Medium'); status=models.CharField(max_length=30,choices=STATUS,default='New'); created_date=models.DateTimeField(auto_now_add=True); due_date=models.DateField(null=True,blank=True)
 def __str__(self): return f'{self.id} - {self.title}'
class Assignment(models.Model):
 work_order=models.ForeignKey(WorkOrder,on_delete=models.CASCADE,related_name='assignments'); vendor=models.ForeignKey(Vendor,on_delete=models.PROTECT,related_name='assignments'); assigned_date=models.DateTimeField(auto_now_add=True); status=models.CharField(max_length=50,default='Assigned')
 class Meta:
